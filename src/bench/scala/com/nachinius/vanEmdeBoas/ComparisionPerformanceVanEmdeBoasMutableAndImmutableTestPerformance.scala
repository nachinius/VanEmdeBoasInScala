...
object ComparisionPerformanceVanEmdeBoasMutableAndImmutableTestPerformance extends Bench.OfflineReport {

//  override def measurer: Measurer[Double] = new scalameter.Measurer.Default

  val seed: Long = 11116793
  val dataSize = Gen.exponential("data size")(1,10000,10)
  val searches: Int = 10

  private val minBit = 16
  private val maxBit = 30
  performance of "successor vs bits (immutable)" in {
    measure method "successor" in {
      val rnd = new Random(seed)
      val bits = Gen.range("bits")(minBit,maxBit,1)
